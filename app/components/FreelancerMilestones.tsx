...
import { Plus, ArrowLeft, Flag, X, Check, Clock, Zap, CircleDot, Ban, TrendingUp, Calendar, DollarSign, OctagonX } from "lucide-react";
...
const STATUS_CONFIG: Record<
    Milestonestatus,
    { label: string; badgeBg: string; badgeBorder: string; badgeText: string; dotColor: string }
> = {
    COMPLETED: {
        label: "COMPLETED",
...
function AddMilestoneModal({ onClose }: { onClose: () => void }) {
    const [form, setForm] = useState({ title: "", subtitle: "", cost: "", deadline: "", description: "" });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            ...
            >
                ...
                <p className="font-sans text-[13px] text-[var(--color-dash-ink3)] leading-relaxed mb-6">
                    Stopping the project will freeze all pending milestones. The freelancer will be notified immediately.
                </p>
    ...
    return (
        <div className="bg-[var(--color-dash-surface1)] border border-[var(--color-dash-border)] rounded-xl p-5 flex flex-col gap-3 hover:border-[var(--color-dash-border-hover)] transition-colors duration-200">
            <div className="flex items-center justify-between">
                <p className="font-mono text-[9px] tracking-[2px] uppercase text-[var(--color-dash-ink3)]">{label}</p>
                <span className="text-[var(--color-dash-ink4)]">{icon}</span>
            </div>
            <p className={`font-serif text-[22px] ${color ?? "text-[var(--color-dash-ink)]"}`}>{value}</p>
        </div>
    );
}
...
export function FreelancerMilestones({ project, projectTitle, projectCode, role }: FreelancerMilestonesProps) {
    ...
    const totalEarned = project.milestones
        ...
        .reduce((acc, m) => acc + m.milestonecost, 0);

    const totalCost = project.milestones.reduce((acc, m) => acc + m.milestonecost, 0);
    const remaining = totalCost - totalEarned;
    const completedPct = totalCost > 0 ? Math.round((totalEarned / totalCost) * 100) : 0;

    const completedCount = project.milestones.filter((m) => m.status === "COMPLETED").length;
    const inProgressCount = project.milestones.filter((m) => m.status === "IN_PROGRESS").length;
    const pendingPayCount = project.milestones.filter((m) => m.status === "PENDING_PAYEMENT").length;
    const notStartedCount = project.milestones.filter((m) => m.status === "NOT_STARTED").length;
    const stoppedCount = project.milestones.filter((m) => m.status === "STOPPED").length;
    ...
    return (
        <div className="w-full min-h-screen bg-[var(--color-dash-bg)] px-4 py-6 lg:px-8 lg:py-8">
            <AnimatePresence>
                ...
                {showStopModal && (
                    <StopProjectModal key="stop" onClose={() => setShowStopModal(false)} />
                )}
            </AnimatePresence>
            ...
            >
                <div>
                    ...
                    <p className="font-mono text-[10px] tracking-[2px] uppercase text-[var(--color-dash-ink3)]">
                        CODE: {projectCode} · Since {formatDate(project.createdAt)}
                    </p>
                </div>
            ...
            <div className="flex flex-col xl:flex-row gap-6">
                <div className="flex flex-col gap-6 xl:w-[340px] xl:shrink-0">
                    <motion.div
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <p className="font-mono text-[9px] tracking-[2px] uppercase text-[var(--color-dash-ink3)]">
                                Progress
                            </p>
                            <span className="font-serif text-[13px] text-[var(--color-dash-green)]">
                                {completedPct}%
                            </span>
                        </div>
                        ...
                        <div className="flex justify-between font-mono text-[9px] tracking-[1px] text-[var(--color-dash-ink4)]">
                    ...
                    >
                        ...
                        <StatCard
                            label="Deadline"
                            value={formatDate(project.deadline, { day: "numeric", month: "short", year: "numeric" })}
                            color={isOverdue ? "text-[var(--color-dash-red)]" : "text-[var(--color-dash-ink)]"}
                    ...
                    <motion.div
                        ...
                        className="bg-[var(--color-dash-surface1)] border border-[var(--color-dash-border)] rounded-xl p-5"
                    >
                        <p className="font-mono text-[9px] tracking-[2px] uppercase text-[var(--color-dash-ink3)] mb-4">
                            Breakdown
                        </p>
                        <div className="flex flex-col gap-2.5">
                            {[
                                { status: "COMPLETED" as Milestonestatus, count: completedCount },
                                { status: "IN_PROGRESS" as Milestonestatus, count: inProgressCount },
                                { status: "PENDING_PAYEMENT" as Milestonestatus, count: pendingPayCount },
                                { status: "NOT_STARTED" as Milestonestatus, count: notStartedCount },
                ...
                >
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <span className="font-mono text-[9px] tracking-[1px] text-[var(--color-dash-ink4)]">
                    ...
                    {project.milestones.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 gap-3">
                            ...
                            <p className="font-mono text-[10px] tracking-[2px] uppercase text-[var(--color-dash-ink4)]">
                                No milestones yet
                            </p>
                        </div>
                    ) : (
